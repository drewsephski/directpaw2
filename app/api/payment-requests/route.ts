...
export async function POST(request: NextRequest) {
  ...
  if (!sitter.stripeAccountId || !(await refreshStripeReadiness(sitter))) return NextResponse.redirect(`${getOrigin()}/dashboard?error=${encodeURIComponent("Complete Stripe onboarding before creating a payment request.")}`, 303);
  ...
  const platformFeeCents = calculatePlatformFeeCents(parsed.data.amount);
  await db()`insert into payment_requests (sitter_id, public_token, amount_cents, platform_fee_cents, description, client_email)
    values (${sitter.id}::uuid, ${token}, ${parsed.data.amount}, ${platformFeeCents}, ${parsed.data.description}, ${parsed.data.clientEmail})`;
