...
export default async function PayPage({
  ...
  return (
    <main className="flex min-h-screen items-center justify-center px-6 py-12">
      <div className="w-full max-w-lg border border-ink bg-white p-8 shadow-[10px_10px_0_#dcebdd]">
        <Link href="/" className="text-lg font-black">
          DirectPaw
        </Link>
        <p className="mt-10 text-xs font-bold uppercase tracking-[.18em] text-leaf">
          Payment to
        </p>
        ...
        {error && (
          <p className="mt-5 border border-coral bg-coral/10 p-3 text-sm">
        ...
        {paymentRequest.status === "open" ? (
          <form
            ...
            className="mt-8"
          >
            <button className="w-full border border-leaf bg-leaf px-6 py-4 text-lg font-bold text-white hover:bg-ink">
              Pay securely with Stripe
            </button>
          </form>
        ) : (
          <p className="mt-8 border border-ink/20 bg-mint p-4 font-bold">
